...
class BWAMEM:
    ...
    def align(self, readfiles: list[pathlib.Path | str], queue: Queue[CandidateAlignment], slsequence: str|None=None, threads=None, this_logger: logging.Logger=logger, filtering: Literal['softclipped'] | Literal['allmatches']='softclipped'):
        if threads is None:
            threads = multiprocessing.cpu_count()-2

        if filtering == 'allmatches' and slsequence is None:
            raise ValueError("When aligning with 'allmatches', the `slsequence` argument must be given.")
        ...
        def line_generator():
            for fname in readfiles:
                ...
                awk_cmd = ['gawk', "-f", pathlib.Path(__file__).parent / f"__assets__/{filtering}.awk"]
                ...
                for line in iter(awk.stdout.readline, ''):
                    sequence_name, position, nr_matched, strand, match_location, sequence = line.split()
