...
def assign_sites(gene_models_gff, slas_pas_sites_gff):
    ...
    for seqreg in gene_models.sequence_regions.values():
        try:
            ...
            logger.warning(f"SLAS/PAS sites GFF does not contain sequence region {seqreg.name} - possible mismatch between slapquant run and given gene models?")
            continue
        for SLAS in (feature for feature in sitesreg.node_registry.values() if feature.type == 'SLAS'):
            geffa.geffa.SLASNode(SLAS.line_nr, seqreg, SLAS.source, 'SLAS', SLAS.start, SLAS.end, '.', SLAS.strand, '.', f"ID={SLAS.attributes['ID']};usage={SLAS.attributes['usage']}")
        for PAS in (feature for feature in sitesreg.node_registry.values() if feature.type == 'PAS'):
            geffa.geffa.PASNode(PAS.line_nr, seqreg, PAS.source, 'PAS', PAS.start, PAS.end, '.', PAS.strand, '.', f"ID={PAS.attributes['ID']};usage={PAS.attributes['usage']}")
        processed_regions.add(seqreg.name)
    sites_only_regions = set(slas_pas.sequence_regions).difference(processed_regions)
    if len(sites_only_regions) > 0:
        logger.warning(f"Gene models GFF does not contain sequence regions present in the SLAS/PAS sites GFF - possible mismatch between slapquant run and given gene models?")
    ...
    for seqreg in gene_models.sequence_regions.values():
        for slas in (node for node in seqreg.node_registry.values() if node.type == 'SLAS'):
            ...
            if len(nodes_to_search) == 0:
                logger.info(f"No CDS found we could match {slas.attributes['ID']} to. Skipping.")
                continue

            closest_node = nodes_to_search[0]

            if closest_node.type == 'PAS':
                logger.info(f"Closest node to {slas.attributes['ID']} is a PAS, no CDS could be assigned.")
            else:
        ...
        for pas in (node for node in seqreg.node_registry.values() if node.type == 'PAS'):
            ...
            if len(nodes_to_search) == 0:
                logger.info(f"No CDS found we could match {pas.attributes['ID']} to. Skipping.")
                continue

            closest_node = nodes_to_search[0]

            if closest_node.type == 'SLAS':
                logger.info(f"Closest node to {pas.attributes['ID']} is a SLAS, no CDS could be assigned.")
            else:
